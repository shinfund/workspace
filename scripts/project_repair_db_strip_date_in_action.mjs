...
// 고장수리DB - "조치내역" 컬럼 앞에 붙은 날짜 문구(예: "26.06.29 ", "26.07월 ")만 제거
...
const DATE_PREFIX_RE = /^\d{2}\.\d{2}(?:\.\d{2})?월?\s+/;
...
(async () => {
  ...
  for (const p of pages) {
    const text = getRichText(p, '조치내역');
    if (!text) continue;
    if (!DATE_PREFIX_RE.test(text)) continue;
    const stripped = text.replace(DATE_PREFIX_RE, '');
    toUpdate.push({ id: p.id, oldText: text, newText: stripped });
  }

  console.log(`[${LABEL}] 총 ${pages.length}건 / 날짜문구 제거 대상 ${toUpdate.length}건`);
  toUpdate.forEach(u => console.log(`  "${u.oldText}" -> "${u.newText}"`));
      ...
      while (idx < toUpdate.length) {
        ...
        try {
          await notionRequest('PATCH', `/v1/pages/${cur.id}`, {
            properties: { '조치내역': { rich_text: [{ text: { content: cur.newText } }] } }
          });
          done++;
        } catch (e) {
          errors++;
