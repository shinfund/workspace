// 장대양봉 Fib되돌림 — 변동폭×Fib 2차원 그리드 최상위후보 견고성 검증 (2026-09-01)
// project_bigcandle_fib_range_grid.mjs에서 변동폭%가 커질수록 perDay가 단조증가(3%→12%, n=5541→212)하는
// 패턴 확인 — 표본축소에 따른 과적합 의심. 최상위 후보(변동폭12%+Fib23.6%, n=212)와 중간 후보
// (변동폭6%+Fib38.2%, n=1558), 기존 확정값(몸통5%+몸통50%, n=585) 3종을 OOS+꼬리의존도로 교차검증.
...
function detectAndSimulate(seq, opts) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    let sizeOk;
    if (opts.filterMode === 'range') sizeOk = (h - l) / l * 100 >= opts.sizePct;
    else sizeOk = (c - o) / o * 100 >= opts.sizePct;
    if (!sizeOk) continue;
...
async function evaluate(label, opts, seqs) {
  ...
  console.log(`  상위15건 종목분산: ${uniqStocks.size}개 종목`);
  console.log(`  연평균 신호수(추정): ${(allTrades.length / (CALENDAR_DAYS / 365)).toFixed(0)}건/년`);
}

async function main() {
  console.error(`[Fib 그리드 최상위후보 견고성 검증] 50종목 로딩 중...`);
  ...
  await evaluate('A. 그리드 최상위(변동폭≥12%+Fib23.6%)', { ...BASE, filterMode: 'range', sizePct: 12, levelMode: 'fib', fibRatio: 0.236 }, seqs);
  await evaluate('B. 중간후보(변동폭≥6%+Fib38.2%)', { ...BASE, filterMode: 'range', sizePct: 6, levelMode: 'fib', fibRatio: 0.382 }, seqs);
  await evaluate('C. 기존 확정값(몸통≥5%+몸통50%)', { ...BASE, filterMode: 'body', sizePct: 5, levelMode: 'body50' }, seqs);
}
