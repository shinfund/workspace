...
async function backtestStock(stock, opts) {
  ...
  for (const ev of events) {
    ...
    // 2026-08-25 신규: 레벨가(L)~TP가(L+step) 구간의 "폭"(step) 자체가 성과와 연관 있는지 —
    // STOP은 항상 L×98%(레벨가 대비 고정 2%)라 리스크는 일정한데, TP까지 거리(step/L%)가 넓을수록
    // 잠재보상은 커지지만 도달까지 더 걸리거나 도달 자체가 어려울 수 있음(손익비 구조 가설).
    const bandWidthPct = ev.step / ev.level * 100;
    trades.push({ name: stock.name, market: stock.market, entryDate: seq[ev.entryIdx].date, level: ev.level, touchCount: ev.touchCount, priorAboveCount: ev.priorAboveCount, uptrend, bandWidthPct, ...res });
  }
...
async function main() {
  ...
  // 2026-08-25 신규: 레벨가~TP가 폭(bandWidthPct = step/L×100) 별 성과 — 5분위 분석
  console.log(`\n═══ 레벨가~TP가 폭(step/L%) 별 성과 ═══`);
  const widths = pooled.map(t => t.bandWidthPct).filter(v => v != null).sort((a, b) => a - b);
  const qAt = p => widths[Math.min(widths.length - 1, Math.floor(p * (widths.length - 1)))];
  const wEdges = [widths[0], qAt(0.2), qAt(0.4), qAt(0.6), qAt(0.8), widths[widths.length - 1]];
  for (let b = 0; b < 5; b++) {
    const lo = wEdges[b], hi = wEdges[b + 1];
    const g = pooled.filter(t => t.bandWidthPct >= lo && (b === 4 ? t.bandWidthPct <= hi : t.bandWidthPct < hi));
    if (!g.length) { console.log(`  ${lo.toFixed(2)}~${hi.toFixed(2)}%: 해당 없음`); continue; }
    ...
    console.log(`  ${lo.toFixed(2)}~${hi.toFixed(2)}%: n=${g.length}  평균 ${avg >= 0 ? '+' : ''}${avg.toFixed(2)}%  승률${win.toFixed(0)}%  TP비율${tpRate.toFixed(0)}%  STOP비율${stopRate.toFixed(0)}%  평균보유${avgDays.toFixed(1)}일  perDay${(avg / avgDays).toFixed(3)}%`);
  }
  console.log('  ※ 폭이 좁을수록(하위20%) 리스크(고정 2%)대비 보상거리가 짧고, 넓을수록(상위20%) 보상거리가 긺(STOP은 항상 L×98%로 고정)');

  console.log(`\n[임계값 스윕] "폭(step/L%) >= X%"만 신호로 채택할 때`);
  for (const th of [0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0]) {
    const g = pooled.filter(t => t.bandWidthPct >= th);
    if (!g.length) continue;
    const win = g.filter(t => t.ret > 0).length / g.length * 100;
    const stopRate = g.filter(t => t.reason === 'STOP').length / g.length * 100;
    const avg = mean(g.map(t => t.ret));
    const avgDays = mean(g.map(t => t.day));
    console.log(`  >=${th.toFixed(1)}%: n=${g.length}(${(g.length / pooled.length * 100).toFixed(0)}%)  승률${win.toFixed(0)}%  STOP비율${stopRate.toFixed(0)}%  평균${avg >= 0 ? '+' : ''}${avg.toFixed(2)}%  평균보유${avgDays.toFixed(1)}일  perDay${(avg / avgDays).toFixed(3)}%`);
  }
  const wide = pooled.filter(t => t.bandWidthPct >= 2.5);
  const wideByStock = {};
  for (const t of wide) wideByStock[t.name] = (wideByStock[t.name] || 0) + 1;
  const wideRanked = Object.entries(wideByStock).sort((a, b) => b[1] - a[1]);
  console.log(`\n[집중도 확인] 폭>=2.5% 신호(n=${wide.length})의 종목 분포 — 특정 종목 쏠림 여부`);
  console.log(`  상위10종목: ${wideRanked.slice(0, 10).map(([n, c]) => `${n}(${c}건)`).join(', ')}`);
  console.log(`  종목 수: ${wideRanked.length}개 종목에 분산 (전체 ${opts.stocks.length}종목 중)`);

  console.log(`\n[임계값 스윕] "폭(step/L%) <= X%"만 신호로 채택할 때(반대방향 확인용)`);
  for (const th of [1.0, 1.5, 2.0, 2.5, 3.0, 5.0, 100]) {
    const g = pooled.filter(t => t.bandWidthPct <= th);
    if (!g.length) continue;
    const win = g.filter(t => t.ret > 0).length / g.length * 100;
    const stopRate = g.filter(t => t.reason === 'STOP').length / g.length * 100;
    const avg = mean(g.map(t => t.ret));
    const avgDays = mean(g.map(t => t.day));
    console.log(`  <=${th.toFixed(1)}%: n=${g.length}(${(g.length / pooled.length * 100).toFixed(0)}%)  승률${win.toFixed(0)}%  STOP비율${stopRate.toFixed(0)}%  평균${avg >= 0 ? '+' : ''}${avg.toFixed(2)}%  perDay${(avg / avgDays).toFixed(3)}%`);
  }
