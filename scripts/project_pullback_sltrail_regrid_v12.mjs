// 눌림목 V3_RETEST SL/TRAIL 재그리드서치 (2026-08-20)
// 배경: v12(지수병행확인+개별ATR%상한6%)로 진입조건이 바뀌어 표본이 761→613건으로 줄고 승률·SL비율이 개선됨.
//       기존 SL8%/TRAIL8%(코스피)·SL18%/TRAIL18%(코스닥)는 v11 이전(구 진입조건) 기준으로 그리드서치된 값이라,
//       진입 품질이 좋아진 v12 표본으로 다시 그리드서치해도 동일값이 최적인지 재확인.
// 원본 project_stock_pullback.mjs(v12 확정판)는 수정하지 않음 — 별도 스윕 스크립트로 검증만 수행.
// 코스닥은 기본 유니버스에 알테오젠 1종목뿐이라 표본 부족 — 코스닥 SL/TRAIL은 현재 확정값(18/18) 고정,
// 코스피만 재그리드서치. 코스닥은 v12 필터 적용 후 표본수만 참고로 출력.
// 사용법: node scripts/project_pullback_sltrail_regrid_v12.mjs [--calendar-days N] [--max-hold N]
...
const STOCK_ATR_CAP = 6; // v12 고정
const TP_PCT = 10, TP_FRAC = 0.5;
const KOSDAQ_SL_FIXED = 18, KOSDAQ_TRAIL_FIXED = 18; // 표본부족으로 현재 확정값 고정
...
async function fetchMarketRegime(p1, p2, symbol) {
  ...
  const idxAtr = buildAtr(chart.high, chart.low, closes, KOSPI_ATR_PERIOD);
  ...
  for (let i = 0; i < dates.length; i++) {
    ...
    volPct[dates[i]] = idxAtr[i] != null ? idxAtr[i] / closes[i] * 100 : null;
  }
...
function computeQualifyArray(seq, marketRegime) {
  ...
  for (let i = MA_LONG + SLOPE_LOOKBACK; i < seq.length - 1; i++) {
    ...
    const idxVol = marketRegime.volPct[s.date];
    if (idxVol == null || idxVol > VOL_CAP) continue;
    if (i < MA_SHORT || s.atrPct == null || s.atrPct <= 0) continue;
    if (s.atrPct > STOCK_ATR_CAP) continue; // v12 고정
...
// v12 고정: 지수병행확인(dualIndex=true), 개별ATR상한6%(computeQualifyArray에 이미 반영), 확인지연없음
function buildEntries(loaded) {
  const entries = [];
  for (const r of loaded) {
    if (r.error || !r.seq) continue;
    for (let i = 0; i < r.qualify.length; i++) {
      if (!r.qualify[i]) continue;
      const otherDate = r.seq[i].date;
      if (r.otherRegime?.regime?.[otherDate] !== true) continue; // v12 지수병행확인
      entries.push({ seq: r.seq, i, date: r.seq[i].date, name: r.name, market: r.market || 'KOSPI' });
    }
  }
...
function fmtRow(label, s) {
  if (!s) return `${label.padEnd(32)} 데이터 없음`;
  return label.padEnd(32) + String(s.n).padStart(6) +
...
async function main() {
  ...
  console.error(`[SL/TRAIL 재그리드서치] ${opts.stocks.length}종목, 최대${opts.maxHold}거래일, 최근${opts.calendarDays}일 (v12 진입조건 고정)`);
  ...
  const entries = buildEntries(valid);
  const kospiEntries = entries.filter(e => e.market !== 'KOSDAQ');
  const kosdaqEntries = entries.filter(e => e.market === 'KOSDAQ');
  console.log(`\n[v12 고정 진입표본] 전체 n=${entries.length} (코스피 ${kospiEntries.length} / 코스닥 ${kosdaqEntries.length})`);

  const SL_GRID = [5, 6, 7, 8, 10];
  const TRAIL_GRID = [5, 6, 7, 8, 10, 12];

  console.log('\n════════ 코스피 SL/TRAIL 재그리드서치 (v12 진입 고정) ════════');
  console.log('(현재 확정값: SL8%/TRAIL8%, v11 이전 진입조건 기준으로 그리드서치된 값)\n');
  console.log('조합'.padEnd(32) + 'n'.padStart(6) + '평균'.padStart(9) + '중앙값'.padStart(9) + '승률'.padStart(7) + 'SL비율'.padStart(7) + 'Sharpe'.padStart(9));
  console.log('─'.repeat(80));

  const rows = [];
  for (const sl of SL_GRID) {
    for (const trail of TRAIL_GRID) {
      const trades = kospiEntries.map(e => simulatePartialTP(e.seq, e.i, e.seq[e.i].close, sl, trail, opts.maxHold, TP_PCT, TP_FRAC)).filter(Boolean);
      const s = summarize(trades);
      rows.push({ label: `SL${sl}/TR${trail}`, s, sl, trail });
    }
  }
  rows.sort((a, b) => (b.s?.sharpe ?? -99) - (a.s?.sharpe ?? -99));
  for (const r of rows) console.log(fmtRow(r.label, r.s));

  const current = rows.find(r => r.sl === 8 && r.trail === 8);
  console.log('\n────────────────────────────────────────');
  console.log('현재 확정값 대비 상위 5개:');
  console.log(fmtRow('SL8/TR8(현재확정)', current?.s));
  for (const r of rows.slice(0, 5)) console.log(fmtRow(r.label, r.s));

  // IS/OOS 검증: 현재확정값 + 그리드 상위 후보
  console.log('\n════════ IS/OOS 견고성 검증 (60/40 분할, 재탐색 없음) ════════\n');
  const top = rows[0];
  const shortlist = [
    { label: 'SL8/TR8(현재확정)', sl: 8, trail: 8 },
    { label: `${top.label}(그리드1위)`, sl: top.sl, trail: top.trail },
  ];
  console.log('후보'.padEnd(24) + '구간'.padEnd(10) + 'n'.padStart(6) + '평균'.padStart(9) + '중앙값'.padStart(9) + '승률'.padStart(7) + 'SL비율'.padStart(7) + 'Sharpe'.padStart(9));
  console.log('─'.repeat(80));
  const splitIdx = Math.floor(kospiEntries.length * 0.6);
  const splitDate = kospiEntries[splitIdx]?.date;
  for (const cand of shortlist) {
    const isE = kospiEntries.filter(e => e.date <= splitDate);
    const oosE = kospiEntries.filter(e => e.date > splitDate);
  ...
  console.log(`  → 코스피 전체표본 n=${kospiEntries.length}, 최근 진입일: ${kospiEntries[kospiEntries.length - 1]?.date}`);

  // 코스닥은 표본부족 참고용만
  console.log('\n════════ 코스닥 참고 (표본부족, 현재확정값 18/18 고정) ════════');
  const kqTrades = kosdaqEntries.map(e => simulatePartialTP(e.seq, e.i, e.seq[e.i].close, KOSDAQ_SL_FIXED, KOSDAQ_TRAIL_FIXED, opts.maxHold, TP_PCT, TP_FRAC)).filter(Boolean);
  console.log(fmtRow(`SL${KOSDAQ_SL_FIXED}/TR${KOSDAQ_TRAIL_FIXED}(코스닥,n=${kosdaqEntries.length})`, summarize(kqTrades)));
}
