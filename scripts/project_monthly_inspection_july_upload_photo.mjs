...
if (!PASSWORD) {
  console.error('사용법: node project_monthly_inspection_july_upload_photo.mjs <Worker비밀번호>');
  process.exit(1);
}
...
function extractSheet(ws, tunnel) {
  // 라벨: (band0=row16/17, band1=31/32, band2=46/47) 1-indexed — 일자행, 사진설명행
  ...
  return Object.values(slots).map(s => ({ ...s, 터널명: tunnel, 제목: `${tunnel} 점검사진` }));
}
...
const all = [];
for (const [sheetName, tunnel] of [['점검사진(주)', '청하터널(주)'], ['점검사진(부)', '청하터널(부)']]) {
  const ws = wb.getWorksheet(sheetName);
  all.push(...extractSheet(ws, tunnel));
}
