// 괴리율(평균회귀) 전략 EMA20돌파 부분익절(LEG20) 삭제 백테스트 — 2026-08-26
// 사용자가 "청산 트리거가 너무 분할돼 있다"며 EMA20 돌파 시 25% 부분익절 단계를 빼고 트리거 단계를 줄이는
// 안을 요청 → 4단계로 축소해 검증: ①손절(sweep) ②진입가대비+20%도달시 50%익절 ③(EMA20 확인 단계 없이)
// 잔량50%는 곧바로 종가<EMA5 하향이탈시 전량청산 ④시간청산20거래일(기존). project_deviation_sl_compromise_backtest.mjs
// (기존 5단계, LEG20 포함)를 스캐폴드로 재사용, 청산 stage machine만 축소.
// 진입조건은 변경 없음(EMA5·EMA20 각각 Z<=-2 & 위치<=3%ile AND EMA50<EMA200), 신호단위(가중평균) 백테스트.
//
// 사용법: node scripts/project_deviation_noleg20_backtest.mjs
...
// 4단계 청산(LEG20 삭제, SL만 파라미터화): SL(sweep) → +20%도달시 50%익절 → 잔량50%는 EMA20 확인 없이
// 곧바로 종가<EMA5 하향이탈시 전량청산 → 시간청산20일. 트레이드 수익률은 레그별 매도비중 가중평균.
function simulateExitNoLeg20(closes, ema5, ema20, entryIdx, slPct, tpPct, maxHold) {
  ...
  for (let i = entryIdx + 1; i < n; i++) {
    ...
    if (stage === 'INIT' && ret >= tpPct) { weightedRet += 0.5 * ret; remaining -= 0.5; stage = 'HOLD'; legs.push('TP20'); }
...
async function main() {
  ...
  console.error('[3/3] LEG20삭제 4단계 청산 sweep 실행 중(TP+20%·시간청산20일 기존 유지)...');
  console.log('\n━━━ EMA20돌파 부분익절(LEG20) 삭제 — SL→+20%50%익절→(EMA20확인없이)잔량50% EMA5하향이탈전량→시간청산20일 ━━━');
  for (const sl of SL_CANDIDATES) {
    const trades = signals.map(s => simulateExitNoLeg20(s.closes, s.ema5, s.ema20, s.idx, sl, TP_PCT, BASE_HOLD)).filter(t => t.reason !== 'OPEN');
