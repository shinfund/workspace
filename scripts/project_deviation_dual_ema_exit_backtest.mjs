// 5EMA+20EMA 이중 괴리율 진입(+장기추세 필터) + 3단계 매도 백테스트 — 스킬: stock-deviation (2026-08-07 최종 확정, 현대차 6~7월 20EMA단독 신호과다 문제 검토용)
// 사용법: node scripts/project_deviation_dual_ema_exit_backtest.mjs [--max-hold N] [--sl N] [--calendar-days N] [--stocks 코드:이름:시장,...]
// 진입(2026-08-07 확정 — 장기추세 필터 추가):
//   EMA5·EMA20 각각 롤링250일 Z<=-2 & 위치<=3%ile("Z+P")을 "동시에"(AND) 만족 AND EMA50 < EMA200(장기 하락추세)인 날 종가 매수 (2026-08-07 스윕 백테스트로 10→3%ile 조임)
//   ※ 장기추세 필터 검토(project_deviation_trend_filter_backtest.mjs) 결과: "장기 상승추세일 때만 매수"라는 최초 가설과 반대로,
//     EMA50<EMA200(하락추세) 구간 신호가 오히려 승률·SL비율 모두 최우수(n=194, 평균+3.90%, 승률80%, SL비율3%, 필터없음 대비 평균+3.49%/승률75%/SL9%)로
//     확인되어 이 방향으로 확정. 2021~2026년 매년 일관되게 재현되고 특정 종목 편중도 없음(상위10종목 합계44%)을 확인한 강건성 검증 완료.
//     평균회귀 전략 특성상 이미 장기하락추세인 종목의 단기 극과매도는 흔한 데드캣바운스로 반등신뢰도가 높은 반면,
//     장기상승추세 종목이 갑자기 단기극과매도까지 빠지는 것은 추세를 깨는 이례적 악재일 가능성이 높아 손절로 이어지기 쉬운 것으로 해석.
// 청산(2026-08-07 확정 — 2단계 구조 시험 후 3단계가 성과 우위라 원복):
//   ① 손절(전량): 진입가 대비 -SL%(기본15) 도달 시 잔여 물량 전량 청산 — 항상 최우선 판정
//   ② 1차 매도(수량50%): 종가가 EMA5를 돌파(종가>=EMA5)한 날 발생
...
//   ※ 2026-08-07 대안(20EMA돌파50%→5EMA하향이탈시 전량, 2단계) 백테스트 결과: 가중평균+3.79%/승률69%/최저-27.17%로 3단계(+4.44%/74%/-21.78%)보다 열세 확인 후 기각.
//     SL(-15%) 발생건수는 두 구조 모두 36건 동일 — 손절률은 청산단계 구조가 아닌 진입신호 품질에 좌우된다는 근거
...
// 시가총액 TOP50 (2026-08-03 KIS 기준, 스크립트11과 동일 유니버스)
...
const ROLL = 250;
const Z_THRESHOLD = -2;
const ENTRY_PCT_THRESHOLD = 3; // 2026-08-07 스윕 백테스트(project_deviation_pct_threshold_sweep.mjs)로 10→3 조임
const FAST_PERIOD = 5;
...
function parseArgs() {
  ...
  // EMA200 워밍업까지 감안해 기본 조회기간을 넉넉히(약 7년) 잡음(트렌드필터 검토 스크립트와 동일 기준)
...
// devKey(예: 'dev5','dev20') 기준 롤링 Z/위치 계산
...
// 3단계 매도 시뮬레이션
// stage: INIT(5EMA돌파 대기) -> LEG5_DONE(20EMA돌파 대기, 잔량50%) -> HOLD(잔량25%, 5EMA하향이탈 감시)
function simulateTrade(seq, i0, opts) {
  const entryClose = seq[i0].close;
  let openWeight = 1.0;
  let stage = 'INIT';
  const legs = [];
  let leg5Done = false, leg20Done = false;
  ...
  return { legs, weightedRet, finalDay, entryDate: seq[i0].date, leg5Done, leg20Done };
}
...
function summarizeTrades(trades) {
  ...
  const neverTouched = trades.filter(t => !t.leg5Done).length;
  const touchedButNo20 = trades.filter(t => t.leg5Done && !t.leg20Done).length;
  const bothLegsDone = trades.filter(t => t.leg5Done && t.leg20Done).length;
  return {
    n: rets.length, avg: mean(rets), med: median(rets), win,
    best: Math.max(...rets), worst: Math.min(...rets), avgDays,
    reasonCount, reasonWeight, neverTouched, touchedButNo20, bothLegsDone,
  };
}
...
async function main() {
  ...
  console.error(`[5EMA+20EMA 이중 괴리율 진입(+장기하락추세 필터) + 3단계 매도 백테스트] ${opts.stocks.length}종목, 손절-${opts.sl}%/최대${opts.maxHold}거래일`);
  console.error(`진입: EMA5·EMA20 각각 Z<=${Z_THRESHOLD} & 위치<=${ENTRY_PCT_THRESHOLD}%ile 동시 충족(AND) AND EMA50<EMA200(장기 하락추세)`);
  console.error(`청산: ①손절-${opts.sl}%(최우선) ②종가≥EMA5 돌파 시 50%매도 ③이후 종가≥EMA20 돌파 시 잔량50%(전체25%)매도 ④이후 종가<EMA5 하향이탈 시 잔량전량매도 ⑤시간청산(${opts.maxHold}거래일)`);
  ...
  console.log(`\n[진행 경로 분류]`);
  console.log(`  EMA5 한 번도 못 돌파(SL/시간청산으로 전량 종료): ${s.neverTouched}건`);
  console.log(`  EMA5 돌파했으나 EMA20은 못 돌파: ${s.touchedButNo20}건`);
  console.log(`  EMA5·EMA20 둘 다 돌파(잔량25% HOLD 진입): ${s.bothLegsDone}건`);

  console.log(`\n━━━ 종목별 신호수 (거래대금·시가 신호과다 여부 확인용) ━━━`);
