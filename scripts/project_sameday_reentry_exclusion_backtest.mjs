// project_4strategy_combined_portfolio_backtest.mjs 포크 — "당일 재진입 제외" 필터 검증용(2026-09-07)
// 배경: LG전자가 같은 날 라운드넘버 청산신호로 매도된 직후 장대양봉 진입신호로 재포착된 사례 발견.
// 검증질문: 어느 전략이든 당일 청산된 종목은 그날 신규진입 후보에서 제외하면 헤드라인 수익률이 개선되는가?
// 사용법: node scripts/project_sameday_reentry_exclusion_backtest.mjs [--from 2019-08-27] [--to ...] [--baseline] [--exclude-reentry]
//   --baseline: 장대양봉 제외, 기존 3전략만(비교기준 재현용)
//   --exclude-reentry: 당일 청산된 종목을 같은 날 신규진입 후보에서 제외
...
function parseArgs() {
  ...
  const o = { from: '2019-08-27', to: null, fetchFrom: '2017-01-01', baseline: false, excludeReentry: false };
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === '--from') o.from = argv[++i];
    if (argv[i] === '--to') o.to = argv[++i];
    if (argv[i] === '--baseline') o.baseline = true;
    if (argv[i] === '--exclude-reentry') o.excludeReentry = true;
  }
  return o;
}
...
function runPortfolioSim(calendar, ctx, includeBigcandle, excludeReentry) {
  ...
  for (let di = 0; di < calendar.length; di++) {
    ...
    for (const s of UNIVERSE) {
      ...
      if (excludeReentry && soldToday.has(s.code)) continue;
...
async function main() {
  ...
  console.error(`[4/4] 시뮬레이션 실행 중(장대양봉 ${opts.baseline ? '제외(기준)' : '포함'}, 당일재진입제외 ${opts.excludeReentry ? 'ON' : 'OFF'})...`);
  const run = runPortfolioSim(calendar, ctx, !opts.baseline, opts.excludeReentry);
  ...
  console.log(`\n━━━ ${opts.baseline ? '기준(3전략만)' : '4전략 통합(눌림목+괴리율+라운드넘버+장대양봉)'} · 당일재진입제외 ${opts.excludeReentry ? 'ON' : 'OFF'} ━━━`);
