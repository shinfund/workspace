...
const GAPS = [
  { date: '2026-06-23', tunnels: ['남정4터널', '남정5터널', '남정6터널', '청하터널', '흥해터널'], title: '2026년 6월 터널전기시설점검표' },
  { date: '2026-05-20', tunnels: ['청하터널', '흥해터널'], title: '2026년 5월 터널전기시설점검표' },
];

async function main() {
  initLog('터널진입차단설비 누락 복원(신규생성)');
  if (!TOKEN) { log('[오류] NOTION_TOKEN 없음'); process.exit(1); }

  let done = 0, err = 0, total = 0;
  for (const gap of GAPS) total += gap.tunnels.length * ITEMS.length;
  log(`[생성 시작] ${total}건`);
