// 라운드넘버 동시신호(CAP=3 초과일) 우선순위 랭킹 고도화 검증 — 2026-09-11
// 배경: [[project_roundnumber_enhancement_4way_rejected]]에서 "신호 자체를 필터링/사이징하는" 4방향이
// 전부 기각(레벨신선도는 신호단위 개선에도 포트폴리오에서 역전)됐음을 확인한 뒤, 성격이 다른 개선안으로
// "이미 확정필터(⑤⑥⑦)를 통과한 신호 중 같은 날 CAP(3)을 넘을 때 어떤 걸 먼저 고를지"만 바꿔보는 실험.
// 총 신호수·전체 필터는 전혀 안 건드리므로 앞선 4방향과 달리 "신호를 늘리거나 줄이는" 효과가 없음 —
// 베타 타이브레이커([[project_stock_factor_score_backtest]])와 동일한 성격의 저위험 변경.
// project_4strategy_combined_portfolio_backtest.mjs 포크. --rn-rank 플래그로 랭킹기준 전환:
//   touch(기존, touchCount desc→aboveCount desc) / recency(ageOfLastTouch asc) /
//   bandwidth(bandWidthPct asc) / composite(정규화 가중합)
// 사용법: node scripts/project_roundnumber_candidate_ranking_portfolio_backtest.mjs [--rn-rank touch|recency|bandwidth|composite]
...
const RN = { WINDOW_DAYS: 150, TARGET_TICKS: 30, RECENT_LOOKBACK: 20, PRIOR_ABOVE_DAYS: 5, MIN_TOUCHES: 3, RECLAIM_WINDOW: 5, STOP_BUFFER_PCT: 3, MAX_HOLD: 60, MIN_ENTRY_POSITION_PCT: 20, MIN_BAND_WIDTH_PCT: 2.5, CAP: 3, RANK: 'touch' };
...
function parseArgs() {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (argv[i] === '--rn-rank') RN.RANK = argv[++i];
  }
  return o;
}
...
function touchInfoBefore(highs, lows, idx, level, windowDays) {
  const lo = Math.max(0, idx - windowDays); let count = 0, lastTouchIdx = -1;
  for (let k = lo; k < idx; k++) if (lows[k] <= level && level <= highs[k]) { count++; lastTouchIdx = k; }
  return { count, ageOfLastTouch: lastTouchIdx >= 0 ? idx - lastTouchIdx : 999 };
}
...
// 확정필터(⑤⑥⑦) 동일 — 이번엔 ev에 bandWidthPct·entryPosition·ageOfLastTouch까지 저장(랭킹 실험용)
function precomputeRoundnumber(st) {
  ...
  for (let i = 1; i < n; i++) {
    ...
    if (bandWidthPct < RN.MIN_BAND_WIDTH_PCT) continue;
    ...
    const touchInfo = touchInfoBefore(highs, lows, i, L, RN.WINDOW_DAYS);
    if (touchInfo.count < RN.MIN_TOUCHES) continue;
      ...
      if (closes[f] >= L) {
        if (closes[f] < L + step) {
          ...
          if (entryPosition >= RN.MIN_ENTRY_POSITION_PCT) {
            events.set(f, { level: L, step, touchCount: touchInfo.count, aboveCount, ageOfLastTouch: touchInfo.ageOfLastTouch, bandWidthPct, entryPosition });
          }
        }
        break;
      }
...
// 랭킹 함수 — RN.RANK에 따라 rnCandsRaw(대기열, ev 포함)를 정렬해 상위 RN.CAP개만 남김
function rankRnCandidates(rnCandsRaw) {
  if (rnCandsRaw.length <= 1) return rnCandsRaw;
  if (RN.RANK === 'touch') {
    return [...rnCandsRaw].sort((a, b) => (b.ev.touchCount - a.ev.touchCount) || (b.ev.aboveCount - a.ev.aboveCount));
  }
  if (RN.RANK === 'recency') {
    return [...rnCandsRaw].sort((a, b) => (a.ev.ageOfLastTouch - b.ev.ageOfLastTouch) || (b.ev.touchCount - a.ev.touchCount));
  }
  if (RN.RANK === 'bandwidth') {
    return [...rnCandsRaw].sort((a, b) => (a.ev.bandWidthPct - b.ev.bandWidthPct) || (b.ev.touchCount - a.ev.touchCount));
  }
  if (RN.RANK === 'composite') {
    // 정규화(0~1, 그날 후보군 내에서만) 후 가중합 — touchCount desc, aboveCount desc, ageOfLastTouch asc, bandWidthPct asc(오늘 세션 발견 반영)
    const touchArr = rnCandsRaw.map(c => c.ev.touchCount), aboveArr = rnCandsRaw.map(c => c.ev.aboveCount);
    const ageArr = rnCandsRaw.map(c => c.ev.ageOfLastTouch), bwArr = rnCandsRaw.map(c => c.ev.bandWidthPct);
    const norm = (v, arr, invert) => { const mn = Math.min(...arr), mx = Math.max(...arr); if (mx === mn) return 0.5; const n = (v - mn) / (mx - mn); return invert ? 1 - n : n; };
    const scored = rnCandsRaw.map(c => ({
      c, score: 0.3 * norm(c.ev.touchCount, touchArr, false) + 0.2 * norm(c.ev.aboveCount, aboveArr, false) + 0.3 * norm(c.ev.ageOfLastTouch, ageArr, true) + 0.2 * norm(c.ev.bandWidthPct, bwArr, true),
    }));
...
function runPortfolioSim(calendar, ctx) {
  ...
  for (let di = 0; di < calendar.length; di++) {
    ...
    const rnRanked = rankRnCandidates(rnCandsRaw);
    const rnCands = rnRanked.slice(0, RN.CAP); skipCount += Math.max(0, rnRanked.length - RN.CAP);
...
async function main() {
  ...
  console.error(`[1/4] 지수·유니버스 시세 로드 중... (RN.RANK=${RN.RANK})`);
  ...
  console.error(`[4/4] 시뮬레이션 실행 중...`);
  const run = runPortfolioSim(calendar, ctx);
  ...
  for (const t of run.trades) {
    ...
    byYear.set(y, (byYear.get(y) || 0) + t.realizedPnl);
  }
  for (const [y, pnl] of [...byYear.entries()].sort()) {
    console.log(`  ${y}: ${fmtWon(pnl)}원 (${run.trades.filter(t => t.exitDate.slice(0,4) === y).length}건)`);
  }
}
