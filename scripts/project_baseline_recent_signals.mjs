...
// 매 실행마다 KRX 전일 시총 기준으로 코스피를 새로 산출한다(2026-08-19, 통합랭킹 폐지).
...
async function main() {
  ...
  console.error(`[기준선] 코스피 ${kospiUniverse.length}종목 스캔 시작(v14: 코스닥 제외, 코스피 전용)`);
  ...
  console.error(`[산출완료] 최근신호·예상종목 각 *_ks.html(코스피전용), 총 4개 fragment 생성`);

  console.log(JSON.stringify({
    generatedAt: new Date().toISOString(), cutoffDate,
    kospi: { signals: ks.stats, watch: watchKs.stats },
  }, null, 2));
}
