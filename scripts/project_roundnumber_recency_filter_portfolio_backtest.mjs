// 라운드넘버 "레벨 신선도(ageOfLastTouch)" 필터 4전략 공유슬롯 포트폴리오 재검증 — 2026-09-11
// 배경: project_roundnumber_enhancement_backtest.mjs(신호단위 풀링 분석)에서 마지막 터치 경과일수가
// 짧을수록(레벨이 최근까지 활발히 작동 중) perDay가 뚜렷이 개선됨을 확인(전체 perDay0.407%→maxAge<=3일
// 필터 시 0.475%, n521→429, 승률은 71%→72%로 유지). 다른 3개 후보(밴드폭비례사이징/트레일링스탑/
// 멀티타임프레임컨플루언스)는 전부 무효 또는 역효과로 이 필터만 포트폴리오 레벨 재검증 대상.
// project_4strategy_combined_portfolio_backtest.mjs 포크, RN 신호탐지에 ageOfLastTouch 계산+
// --rn-max-age 플래그 추가(기본 Infinity=기존과 동일, 3 지정 시 필터 적용).
// 사용법: node scripts/project_roundnumber_recency_filter_portfolio_backtest.mjs [--rn-max-age 3]
...
const RN = { WINDOW_DAYS: 150, TARGET_TICKS: 30, RECENT_LOOKBACK: 20, PRIOR_ABOVE_DAYS: 5, MIN_TOUCHES: 3, RECLAIM_WINDOW: 5, STOP_BUFFER_PCT: 3, MAX_HOLD: 60, MIN_ENTRY_POSITION_PCT: 20, MIN_BAND_WIDTH_PCT: 2.5, CAP: 3, MAX_TOUCH_AGE: Infinity };
...
function parseArgs() {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (argv[i] === '--rn-max-age') RN.MAX_TOUCH_AGE = parseFloat(argv[++i]);
  }
  return o;
}
...
// 터치카운트 + 마지막 터치 경과일(ageOfLastTouch) 함께 반환
function touchInfoBefore(highs, lows, idx, level, windowDays) {
  ...
  return { count, ageOfLastTouch: lastTouchIdx >= 0 ? idx - lastTouchIdx : Infinity };
}
...
function precomputeRoundnumber(st) {
  ...
  for (let i = 1; i < n; i++) {
    ...
    if (touchInfo.ageOfLastTouch > RN.MAX_TOUCH_AGE) continue; // 신규: 레벨 신선도 필터
    for (let f = i; f < Math.min(n, i + RN.RECLAIM_WINDOW); f++) {
      if (closes[f] < L - step) break;
      if (closes[f] >= L) {
        if (closes[f] < L + step) {
          const entryPosition = (closes[f] - L) / step * 100;
          if (entryPosition >= RN.MIN_ENTRY_POSITION_PCT) events.set(f, { level: L, step, touchCount: touchInfo.count, aboveCount });
        }
        break;
      }
    }
...
async function main() {
  ...
  console.error(`[1/4] 지수·유니버스 시세 로드 중... (RN.MAX_TOUCH_AGE=${RN.MAX_TOUCH_AGE})`);
  ...
  console.log(`\n━━━ 4전략 통합 (RN.MAX_TOUCH_AGE=${RN.MAX_TOUCH_AGE}) ━━━`);
