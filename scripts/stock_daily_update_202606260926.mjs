...
// ── 시장 컨텍스트 (KOSPI/KOSDAQ 등락률) ─────────────────────
function fetchMarketContext() {
  const fetchIndex = (code) => new Promise(resolve => {
    const req = https.request({
      hostname: 'm.stock.naver.com',
      path: `/api/index/${code}/basic`,
      method: 'GET',
      ...
      res.on('end', () => {
        try {
          ...
          resolve({
            지수: parseFloat((j.closePrice || '0').replace(/,/g, '')),
            등락률: parseFloat(j.fluctuationsRatio || '0'),
            전일대비: parseFloat((j.compareToPreviousClosePrice || '0').replace(/,/g, '')),
          });
        } catch { resolve(null); }
      });
    });
  ...
  return Promise.all([fetchIndex('KOSPI'), fetchIndex('KOSDAQ')])
    .then(([kospi, kosdaq]) => ({ kospi, kosdaq }))
    .catch(() => ({ kospi: null, kosdaq: null }));
}
...
// ── 이슈 분석 (Haiku 배치 → 핵심/강함 Sonnet 심층) ──────────
async function analyzeIssues(stocks, newsMap, marketCtx) {
  ...
  const marketLine = (() => {
    ...
    return `시장 현황: KOSPI ${fmt(k?.등락률)}, KOSDAQ ${fmt(q?.등락률)}`;
  })();
  ...
  // ── Step 1: Haiku 배치 (전체 1차 분류) ─────────────────────
  const batchPrompt = `오늘(${todayStr}) 한국 주식시장 거래대금 상위 종목입니다.${marketLine ? ` ${marketLine}.` : ''} 각 종목을 분석하여 JSON 배열만 반환하세요.
...
출력 규칙: JSON 배열만, compact 한 줄 형식. 마크다운 금지. 줄바꿈 금지.`;
  ...
  try {
    ...
    log(`[Phase 3] Haiku 배치 오류: ${e.message} — 규칙 기반으로 대체`);
    return ruleBasedAnalysis(stocks);
  }

  // ── Step 2: 핵심/강함 → Sonnet 심층 분석 ───────────────────
  ...
  if (highTier.length > 0) {
    log(`[Phase 3] 고강도 ${highTier.length}종목 Sonnet 심층 분석 중...`);
    for (const s of highTier) {
      const news = newsMap[s.종목코드] || { titles: [] };
      const prev = aiMap[s.종목코드];
      const deepPrompt = `다음 종목을 심층 분석해주세요.${marketLine ? `\n${marketLine}` : ''}
...
출력 규칙: JSON 객체 1개만. 마크다운 금지. 줄바꿈 금지.`;
      try {
        ...
        const stripped = raw.replace(/```[a-z]*\n?/gi, '').replace(/```/g, '').trim();
        const obj = JSON.parse(fixJsonNewlines(stripped));
        aiMap[s.종목코드] = { ...obj, model: 'Sonnet' };
        log(`  [Sonnet] ${s.종목명} 심층 분석 완료`);
      } catch(e) {
        log(`  [Sonnet] ${s.종목명} 분석 실패: ${e.message} — Haiku 결과 유지`);
      }
    }
  }
...
async function main() {
  ...
  let kisRaw, krxDayRaw, marketCtx;
  try {
    [kisRaw, krxDayRaw, marketCtx] = await Promise.all([
  ...
  if (marketCtx?.kospi || marketCtx?.kosdaq) {
    const k = marketCtx.kospi, q = marketCtx.kosdaq;
    const fmt = v => v != null ? `${v >= 0 ? '+' : ''}${v.toFixed(2)}%` : '-';
    log(`[Phase 1] 시장지수 — KOSPI ${fmt(k?.등락률)} (${k?.지수?.toFixed(2) ?? '-'}), KOSDAQ ${fmt(q?.등락률)} (${q?.지수?.toFixed(2) ?? '-'})`);
  } else {
    log('[Phase 1] 시장지수 수집 실패 — 프롬프트에서 생략');
  }
  ...
  log(`[Phase 3] Claude AI 분석 중... (${ANTHROPIC_API_KEY ? 'Haiku 배치 → 핵심/강함 Sonnet 심층' : '규칙 기반 fallback'})`);
  const analyses = await analyzeIssues(kisFiltered, newsMap, marketCtx);
