// 눌림목 V3_RETEST 진입/청산 재정비 그리드서치 (2026-08-20)
// 배경: 확정 v11(SL8%/TRAIL8%, 승률56%, Sharpe0.407)에 대해 "청산 승률이 낮고 손절비율이 높다"는 지적으로
//       재검증 요청. 과거 이미 기각된 항목(ATR%정규화 SL/TRAIL, EMA5/20 청산대체, RSI·거래량 필터)은 재시도하지 않고,
//       스킬 "미해결 과제"에 남아있던 미시도 항목만 조합 테스트:
//       ① 진입 1일 확인지연(거짓돌파 방지, 신호일+확인일 연속충족 시 확인일 종가로 진입)
//       ② 개별종목 자체 ATR% 상한(과변동성 종목 배제)
//       ③ SL/TRAIL 고정값 재그리드(v2 그리드서치는 REGIME_STREAK_MIN·VOL_CAP 필터 추가 이전 결과라 재확인)
// 원본 project_stock_pullback.mjs(v11 확정판)는 수정하지 않음 — 별도 스윕 스크립트로 검증만 수행.
// 사용법: node scripts/project_pullback_entry_exit_retune_sweep.mjs [--calendar-days N] [--max-hold N]
...
// 일자별 V3_RETEST 전 조건(진입확인지연·개별ATR상한 제외) 충족여부 boolean 배열 생성
...
// confirmDelay: 0=신호당일 진입(v11 기존방식) / 1=신호일+다음날 연속충족 시 다음날 종가 진입
// atrCapStock: null=무제한, N=개별종목 ATR%가 N 초과면 진입 스킵
// dualIndex: false=자기 시장 지수만 사용(v11 기존방식) / true=반대쪽 지수도 상승국면이어야 진입(시장 전체 국면필터 강화)
function buildEntries(loaded, opts) {
  const entries = [];
  for (const r of loaded) {
    ...
    for (let i = 0; i < r.qualify.length; i++) {
      ...
      let entryIdx = i;
      if (opts.confirmDelay === 1) {
        if (!r.qualify[i + 1]) continue; // 확인일도 조건 충족해야 진입
        entryIdx = i + 1;
      }
      if (opts.atrCapStock != null) {
        const atrPct = r.seq[entryIdx].atrPct;
        if (atrPct == null || atrPct > opts.atrCapStock) continue;
      }
      if (opts.dualIndex) {
        const otherDate = r.seq[entryIdx].date;
        if (r.otherRegime?.regime?.[otherDate] !== true) continue;
      }
      entries.push({ seq: r.seq, i: entryIdx, date: r.seq[entryIdx].date, name: r.name, market: r.market || 'KOSPI' });
    }
  }
  // confirmDelay=1일 때 같은 종목·인접일 신호가 중복 진입되지 않도록 날짜순 정렬 후 스킵 처리는 생략(v11도 동일하게 중복 허용 안 함 — 원본 로직상 신호는 눌림구간에서만 발생해 자연히 희소)
...
function fmtRow(label, s) {
  if (!s) return `${label.padEnd(40)} 데이터 없음`;
  return label.padEnd(40) + String(s.n).padStart(6) +
...
async function main() {
  ...
  console.error(`[눌림목 진입/청산 재정비 스윕] ${opts.stocks.length}종목, 최대${opts.maxHold}거래일, 최근${opts.calendarDays}일`);

  const p2 = Math.floor(Date.now() / 1000);
  const p1 = p2 - opts.calendarDays * 24 * 3600;
  // dualIndex(코스닥지수 병행 확인) 스윕을 위해 KOSDAQ지수는 항상 조회
  ...
  const DUAL_INDEX = [false, true];
  const CONFIRM_DELAYS = [0, 1];
  const ATR_CAPS = [null, 6, 8];
  const SL_TRAIL = [[8, 8], [10, 10], [6, 12], [10, 15]];

  console.log('\n════════ 눌림목 진입/청산 재정비 그리드서치 결과 ════════');
  console.log('(baseline = v11 확정: 지수단일·확인지연없음·ATR상한없음·SL8%/TRAIL8%)');
  console.log('총 조합수:', DUAL_INDEX.length * CONFIRM_DELAYS.length * ATR_CAPS.length * SL_TRAIL.length, '\n');
  console.log('조합'.padEnd(48) + 'n'.padStart(6) + '평균'.padStart(9) + '중앙값'.padStart(9) + '승률'.padStart(7) + 'SL비율'.padStart(7) + 'Sharpe'.padStart(9));
  console.log('─'.repeat(95));

  const rows = [];
  for (const dualIndex of DUAL_INDEX) {
    for (const confirmDelay of CONFIRM_DELAYS) {
      for (const atrCapStock of ATR_CAPS) {
        const entries = buildEntries(valid, { confirmDelay, atrCapStock, dualIndex });
        for (const [sl, trail] of SL_TRAIL) {
          const trades = entries.map(e => simulatePartialTP(e.seq, e.i, e.seq[e.i].close, sl, trail, opts.maxHold, TP_PCT, TP_FRAC)).filter(Boolean);
          const s = summarize(trades);
          const label = `지수${dualIndex ? '병행' : '단일'}·확인${confirmDelay}일·ATR상한${atrCapStock ?? '없음'}·SL${sl}/TR${trail}`;
          rows.push({ label, s, dualIndex, confirmDelay, atrCapStock, sl, trail });
        }
      }
    }
  }
  ...
  const baseline = rows.find(r => !r.dualIndex && r.confirmDelay === 0 && r.atrCapStock == null && r.sl === 8 && r.trail === 8);
  console.log('\n────────────────────────────────────────');
  console.log('baseline(v11) 대비 상위 5개 조합:');
  console.log(fmtRow('baseline(v11)', baseline?.s));
  for (const r of rows.slice(0, 5)) console.log(fmtRow(r.label, r.s));

  // 요인별 단독 개선 여부(다른 요인은 baseline 고정) — 어느 레버가 실제로 효과 있는지 분리 확인
  console.log('\n────────────────────────────────────────');
  console.log('요인별 단독효과(다른 조건은 baseline 유지):');
  const soloDual = rows.find(r => r.dualIndex && r.confirmDelay === 0 && r.atrCapStock == null && r.sl === 8 && r.trail === 8);
  const soloConfirm = rows.find(r => !r.dualIndex && r.confirmDelay === 1 && r.atrCapStock == null && r.sl === 8 && r.trail === 8);
  const soloAtr6 = rows.find(r => !r.dualIndex && r.confirmDelay === 0 && r.atrCapStock === 6 && r.sl === 8 && r.trail === 8);
  const soloAtr8 = rows.find(r => !r.dualIndex && r.confirmDelay === 0 && r.atrCapStock === 8 && r.sl === 8 && r.trail === 8);
  console.log(fmtRow('①지수병행만 추가', soloDual?.s));
  console.log(fmtRow('②확인1일지연만 추가', soloConfirm?.s));
  console.log(fmtRow('③개별ATR상한6%만 추가', soloAtr6?.s));
  console.log(fmtRow('③개별ATR상한8%만 추가', soloAtr8?.s));

  // ── IS/OOS 견고성 확인: 그리드 1위를 그대로 채택하지 않고 표본유지 후보들까지 함께 검증 ──
  console.log('\n════════ IS/OOS 견고성 검증 (60/40 분할, 재탐색 없음) ════════\n');
  const shortlist = [
    { label: 'baseline(v11)', o: { confirmDelay: 0, atrCapStock: null, dualIndex: false }, sl: 8, trail: 8 },
    { label: '①지수병행만', o: { confirmDelay: 0, atrCapStock: null, dualIndex: true }, sl: 8, trail: 8 },
    { label: '③ATR상한6%만', o: { confirmDelay: 0, atrCapStock: 6, dualIndex: false }, sl: 8, trail: 8 },
    { label: '①+③(지수병행+ATR상한6%)', o: { confirmDelay: 0, atrCapStock: 6, dualIndex: true }, sl: 8, trail: 8 },
    { label: '①+②+③ 그리드1위', o: { confirmDelay: 1, atrCapStock: 6, dualIndex: true }, sl: 8, trail: 8 },
  ];
  ...
  for (const cand of shortlist) {
    const entries = buildEntries(valid, cand.o);
    ...
    const isS = summarize(isE.map(e => simulatePartialTP(e.seq, e.i, e.seq[e.i].close, cand.sl, cand.trail, opts.maxHold, TP_PCT, TP_FRAC)).filter(Boolean));
    const oosS = summarize(oosE.map(e => simulatePartialTP(e.seq, e.i, e.seq[e.i].close, cand.sl, cand.trail, opts.maxHold, TP_PCT, TP_FRAC)).filter(Boolean));
    console.log(fmtRow(`${cand.label} / IS`, isS));
    console.log(fmtRow(`${cand.label} / OOS`, oosS));
    const lastDate = entries[entries.length - 1]?.date;
    console.log(`  → 최근 진입일: ${lastDate}, 전체표본 n=${entries.length}`);
  }
}
