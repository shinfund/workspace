...
// 사용법: node scripts/project_portfolio_integrated_exit_check.mjs --price=live|close
...
function parsePriceMode() {
  ...
  if (mode !== 'live' && mode !== 'close') {
    console.error('사용법: node project_portfolio_integrated_exit_check.mjs --price=live|close');
...
async function fetchKisPriceMap(codes) {
  ...
  console.error(`[KIS] 당일 ${PRICE_MODE === 'live' ? '실시간 현재가' : '정규장 확정 종가'} ${map.size}/${codes.length}종목 확보`);
  return map;
}
