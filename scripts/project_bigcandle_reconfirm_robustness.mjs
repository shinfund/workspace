// 장대양봉 눌림+재돌파(확인창5일) 변형 — 견고성 검증 (2026-09-01)
// project_bigcandle_pullback_reconfirm_backtest.mjs 1차 스윕에서 confirmWindow=5가
// n=964(baseline 대비 48%)로 표본을 어느정도 유지하면서 perDay+0.317%(baseline +0.291% 대비 개선)로
// 균형점을 보여 유력 후보로 선정. baseline(즉시터치매수)과 동일한 방법론(OOS 시간분할+꼬리의존도)으로
// 견고성 확인.
...
const OPTS = { calendarDays: 2555, bodyPct: 4, retestWindow: 20, stopBufferPct: 0.5, maxHold: 15, requireUptrend: true, confirmWindow: 5 };
...
async function main() {
  console.error(`[재돌파확인(확인창5일) 견고성 검증] 확정후보: 몸통4%↑/터치후5일내 터치일고가 종가재돌파매수/STOP99.5%/최대15일/상승국면필터`);
  ...
  console.log(`\n━━━ OOS 시간분할(분기점 ${midDate}) ━━━`);
  console.log(fmtRow('전반부', statOf(fh)));
  console.log(fmtRow('후반부', statOf(sh)));

  const sorted = [...allTrades].sort((a, b) => b.ret - a.ret);
  console.log(`\n━━━ 꼬리의존도 ━━━`);
  const cuts = [['상위5건', 5], ['상위10건', 10], ['상위5%', Math.round(allTrades.length * 0.05)], ['상위10%', Math.round(allTrades.length * 0.10)]];
  for (const [label, k] of cuts) console.log(fmtRow(`${label} 제거`, statOf(sorted.slice(k))));
  ...
  console.log(`\n━━━ 상위 15건(종목분산 확인) ━━━`);
  for (const t of sorted.slice(0, 15)) console.log(`${t.name}\t${t.entryDate}\t+${t.ret.toFixed(2)}%\t${t.day}일\t${t.reason}`);
}
